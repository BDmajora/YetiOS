...
def run_stage(cfg: Config, loop: "str | None") -> None:
    ...
    print(f"      -drive file={cfg.img_path},format=raw,if=ide \\")
    ...
    print()
    print("NOTE: OVMF lacks virtio-block drivers, so the disk MUST use if=ide")
    print("for the firmware to see the ESP. On first boot from a fresh image,")
    print("OVMF finds \\EFI\\BOOT\\BOOTX64.EFI automatically. After the first")
    print("successful boot, the libreldr-register service writes a permanent")
    print("NVRAM entry for subsequent boots.")